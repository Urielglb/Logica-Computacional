...
instance Show MyException  where
    show FAIL = " Sustitución no legal"
...
apsubF a@(All nom form) s
    | not (isInSubst nom s) = All nom (apsubF form s)
    | otherwise = throw FAIL
apsubF e@(Ex nom form) s
    | not (isInSubst nom s) = Ex nom (apsubF form s)
    | otherwise = throw FAIL

isInSubst :: Nombre -> Subst -> Bool
isInSubst nom s = or [isInNomTerm nom x | x<-s]

isInNomTerm :: Nombre->(Nombre, Term) -> Bool
isInNomTerm nom (ns, ts) = or [nom==ns, or [t==nom | t<-varT ts]]

x = V "x"
a = (All "x"(Conj (Pr "Q"[V "m"]) (Pr "P"[V "w"])))
s = [("w", V "z"),("m", F "a"[V "t"])]
ex = (Ex "x"(Conj (Pr "Q"[V "x"]) (Pr "P"[V "w"])))
s2 = [("w", V "x"),("m", F "a"[V "t"])]
